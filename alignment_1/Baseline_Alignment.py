...
if __name__ == "__main__":
    ...
    # Remove cache directory if specified
    if os.path.exists(cache_dir) and args.delete_cache:
    ...
    for i, (piece_name, pdata) in enumerate(dataset.items()):
        ...
        cache_fn = os.path.join(cache_dir, f"{piece_name}.npz")

        # Load a cached alignment
        if os.path.exists(cache_fn):
            alignment_info = dict(np.load(cache_fn, allow_pickle=True))
            predicted_alignment = alignment_info[piece_name]
        ...
        # The comparison only makes sense for the training set.
        # The test set will always return 0 because there is no ground truth
