...
def get_performance_info(piece, performer):
    ...
    musicxml_fn = os.path.join(
        data.DATASET_DIR, "musicxml", "{}.musicxml".format(piece)
    )
    match_fn = os.path.join(
        data.DATASET_DIR, "match", "{}_{}.match".format(piece, performer)
    )

    part = partitura.load_musicxml(musicxml_fn)[0]

    perf, alignment = partitura.load_match(match_fn, first_note_at_zero=True)
    ppart = perf[0]
...
def show_performance(piece, performer, fig, axs, keep_zoom):
    ...
    targets, snote_ids = PERF_CODEC.encode(
        part=part,
        ppart=ppart,
        alignment=alignment,
    )
    ...
    s_to_p_id = dict(
        (a["score_id"], a["performance_id"]) for a in alignment if a["label"] == "match"
    )
    ...
    score_perf_map = interp1d(
        s_times, p_times, bounds_error=False, fill_value=(p_times[0], p_times[-1])
    )
...
def performance_player():
    ...
    performer_dd.observe(
        partial(update_current_perf, item="performer"), names=["value"]
    )
    ...
    def on_mouse_up(event):
        ...
        for ax in axs:
            cursor.append(
                ax.fill_betweenx(
                    ax.get_ylim(), play_range[0], play_range[1], alpha=0.2, color="gray"
                )
            )
