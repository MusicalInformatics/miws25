...
OUT_DIR = Path(tempfile.gettempdir()) / "trained_models"
def construct_model(config, in_names, out_names, out_dir):
    ...
    model_name = (
        "-".join(out_names) + "-" + ("onsetwise" if config["onsetwise"] else "notewise")
    )
    ...
    json.dump(jsonize_dict(model_cfg), open(config_out, "w"), indent=2)
    model = c_model(model_cfg)

    return model, model_out_dir


def setup_output_directory(out_dir=OUT_DIR):
...
def split_datasets_by_piece(datasets, test_size=0.2, valid_size=0.2):
    ...
    return (ConcatDataset(train_set), ConcatDataset(valid_set), ConcatDataset(test_set))
...
def split_datasets(datasets, test_size=0.2, valid_size=0.2):
    ...
    return (
        ConcatDataset([datasets[i] for i in train_idxs]),
        ConcatDataset([datasets[i] for i in valid_idxs]),
        ConcatDataset([datasets[i] for i in test_idxs]),
    )


def train_model(model, train_set, valid_set, config, out_dir):
