...
def plot_basis(basis, names, onsets=None, title=None):
    ...
    fig, axs = plt.subplots(n_basis, sharex=True, gridspec_kw={"hspace": 0})
    if n_basis == 1:
        axs = [axs]
...
def plot_predictions_and_targets(predictions, targets):
    ...
    for i, pn in enumerate(param_names):
        axs[i].plot(predictions[pn], color="firebrick", label="predictions")
...
def plot_predictions(
    ...
    if param_names is None:
        param_names = predictions.dtype.names
    fig, axs = plt.subplots(len(param_names), sharex=True, gridspec_kw={"hspace": 0.15})
