...
class NNTrainer(ABC):
    ...
    def compute_data_stats(self):
        ...
        for b_idx, (input, target) in bar:
            ...
            if self.device is not None:
                input = input.type(self.dtype).to(self.device)
                target = target.type(self.dtype).to(self.device)
    ...
    def train(self):
        ...
        LOGGER.info(
            "valid loss before training:" + valid_losses.last_loss + " r2:" + str(r2)
        )
...
class SupervisedTrainer(NNTrainer):
    ...
    def valid_step(self, *args, **kwargs):
        ...
        return np.mean(losses, axis=0), np.mean(
            np.ma.masked_invalid(correlations), axis=0
        )
