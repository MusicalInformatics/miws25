...
def make_datasets(
    ...
    all_targets = list(
        set([n for model_spec in model_specs for n in model_spec["parameter_names"]])
    )
    ...
    for piece, files in pair_files(folders, by_prefix=not quirks).items():
        ...
        for match in files["match"]:
            ...
            perf, alignment = load_match(match, first_note_at_zero=True)
            ppart = perf[0]
